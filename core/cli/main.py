...
from core.ui.base import UIBase, UIClosedError, UserInput, Aipa_source
...
async def run_project(sm: StateManager, ui: UIBase) -> bool:
    ...
    try:
        ...
        await ui.send_message(
            f"Stopping Aipa due to an error while calling the LLM API: {err.message}",
            source=Aipa_source,
        )
...
async def start_new_project(sm: StateManager, ui: UIBase) -> bool:
    ...
    while True:
        ...
        if not project_name:
            await ui.send_message("Please choose a project name", source=Aipa_source)
        elif len(project_name) > 100:
            await ui.send_message("Please choose a shorter project name", source=Aipa_source)
        else:
            break
...
async def run_Aipa_session(sm: StateManager, ui: UIBase, args: Namespace):
    """
    Run a Aipa session.
    ...
    if not args.no_check:
        if not await llm_api_check(ui):
            await ui.send_message(
                "Aipa cannot start because the LLM API is not reachable.",
                source=Aipa_source,
            )
            return False
...
async def async_main(
    ...
    telemetry.start()
    success = await run_Aipa_session(sm, ui, args)
