...
WARN_FRAMEWORKS_URL = "https://github.com/Aipa-io/aipa/wiki/Using-aipa-with-frontend-frameworks"
...
class Architect(BaseAgent):
    ...
    async def select_templates(self, spec: Specification) -> tuple[str, dict[ProjectTemplateEnum, Any]]:
        ...
        Although the Aipa database models support multiple projects, this
    ...
    async def check_compatibility(self, arch: Architecture) -> bool:
        ...
        if warn_system_deps:
            await self.ask_question(
                f"Warning: Aipa doesn't officially support {', '.join(warn_system_deps)}. "
        ...
        if warn_package_deps:
            await self.ask_question(
                f"Warning: Aipa works best with vanilla JavaScript. "
