...
class Orchestrator(BaseAgent):
    ...
    async def offline_changes_check(self):
        """
        Check for changes outside Aipa.
        ...
        if self.state_manager.workspace_is_empty():
            ...
            hint = "".join(
                [
                    "If you would like Aipa to import those changes, click 'Yes'.\n",
                    "Clicking 'No' means Aipa will restore (overwrite) all files to the last stored state.\n",
                ]
            )
            use_changes = await self.ask_question(
                ...
                buttons={
                    ...
                    "no": "No, restore last Aipa state",
                },
                buttons_only=True,
                hint=hint,
            )
            if use_changes.button == "yes":
                log.debug("Importing offline changes into Aipa.")
    ...
    async def handle_done(self, agent: BaseAgent, response: AgentResponse) -> AgentResponse:
        ...
        This also checks for any files created or modified outside Aipa and
        ...
        # If there are any new or modified files changed outside Aipa,
    ...
    def create_agent(self, prev_response: Optional[AgentResponse]) -> Union[List[BaseAgent], BaseAgent]:
        ...
        if state.unfinished_iterations:
            ...
            if current_iteration_status == IterationStatus.HUNTING_FOR_BUG:
                ...
                # Aipa cannot solve the issue so we're starting pair programming
                return BugHunter(self.state_manager, self.ui)
